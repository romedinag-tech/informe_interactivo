...
const GLOSARIO: { term: string; definition: string; source?: string }[] = [
  { term: "IMIV", definition: "Informe de Mitigación de Impacto Vial: estudio que evalúa los efectos de un proyecto sobre el sistema de transporte y define medidas de mitigación.", source: "Ley 20.958; DS 30/2019 MTT-MINVU" },
  { term: "SECTRA", definition: "Programa de Vialidad y Transporte Urbano del Ministerio de Transportes; genera y administra los modelos de transporte de las ciudades chilenas.", source: "MTT" },
  { term: "EOD", definition: "Encuesta Origen–Destino de viajes: levantamiento de los viajes de los hogares por propósito, modo, hora y zona.", source: "SECTRA" },
  { term: "TMDA", definition: "Tránsito Medio Diario Anual: promedio de vehículos que circulan por un punto de la red en un día, promediado sobre el año.", source: "Manual de Carreteras, MOP" },
  ...
  { term: "NSE", definition: "Nivel socioeconómico: clasificación de los hogares según ingreso y características, usada para segmentar la demanda.", source: "AIM / CASEN" },
  { term: "GEH", definition: "Estadístico de bondad de ajuste que compara flujos modelados con observados; valores bajo 5 indican buen ajuste.", source: "WebTAG, UK DfT" },
...
async function main() {
  ...
  const report = await prisma.report.create({
    data: {
      ...
      glossary: {
        create: GLOSARIO.map((g) => ({ term: g.term, definition: g.definition, source: g.source ?? null })),
      },
      chapters: {
